...
class BMSEnv(gym.Env):
    ...
    def __init__(self, data_handler, current_optimizer, params_ins, journalist, verbose=True, **kwargs):
        ...
        self.current_time_saving = datetime.datetime.now()
    ...
    def _get_reward(self, p_loss_step, delta_soh, action):
        ...
        # delta_soh = 0.005 / delta_soh = 0.002
        """
        batt_wear_cost = constant_value * delta_soh 
        batt_wear_cost_sum = \sigma{constant_value * delta_soh} | 0 to 1 year
        bat_wear_cost_gian - batt_wear_cost_Dan = const_value * (\sigma{delta_soh_gian} - \sigma{delta_soh_Dan})
...
# =============================== Debugging ===============================
    ...
    def append_to_csv(self, filepath="results/csvs"):
        """
        ...
        """
        # Handle when filepath is passed as a list (from SubprocVecEnv env_method)
        if isinstance(filepath, list) and len(filepath) > 0:
            filepath = filepath[0]
            
        # Create default path if not provided
        formatted_time = self.current_time_saving.strftime('%Y-%m-%d_%H-%M-%S')
        if filepath is None:
            # Create directories if they don't exist
            os.makedirs("./results/csv", exist_ok=True)
            filepath = f"./results/csv/step_data_{formatted_time}_{self.episode_number}.csv"
