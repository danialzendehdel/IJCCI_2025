...
def run_optimization(dataset_name, R_lookup, SOC_lookup, C_rate_lookup, OCV_lookup, config):
    ...
    for ind in range(num_steps):
        ...
        mode = "charging" if new_I > 0 else "discharging"
...
def compare_and_save_results(nmc_results, nasa_results):
    ...
    # Save to CSV
    nmc_df.to_csv("comparison_results/nmc_results.csv", index=False)
    nasa_df.to_csv("comparison_results/nasa_results.csv", index=False)
    comparison_df.to_csv("comparison_results/comparison_analysis.csv", index=False)
...
def create_comparison_plots(nmc_df, nasa_df, comparison_df):
    ...
    plt.title('SOC Comparison between NMC and NASA Data', fontsize=16)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    plt.savefig("comparison_results/soc_comparison.png", dpi=300, bbox_inches='tight')
    ...
    plt.title('Resistance Comparison between NMC and NASA Data', fontsize=16)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    plt.savefig("comparison_results/resistance_comparison.png", dpi=300, bbox_inches='tight')
    ...
    plt.title('Current Comparison between NMC and NASA Data', fontsize=16)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    plt.savefig("comparison_results/current_comparison.png", dpi=300, bbox_inches='tight')
    
    # Plot 4: Power profile with charge/discharge indication
    plt.figure(figsize=(12, 8))
    plt.bar(nmc_df.index, nmc_df['power'], color=['g' if i > 0 else 'r' for i in nmc_df['power']])
    plt.axhline(y=0, color='k', linestyle='-', alpha=0.3)
    plt.xlabel('Step', fontsize=14)
    plt.ylabel('Power (kW)', fontsize=14)
    plt.title('Input Power Profile (Green=Charging, Red=Discharging)', fontsize=16)
    plt.grid(True, alpha=0.3)
    plt.savefig("comparison_results/power_profile_colored.png", dpi=300, bbox_inches='tight')
    ...
    plt.title('Percentage Differences (NASA vs NMC)', fontsize=16)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    plt.savefig("comparison_results/percentage_differences.png", dpi=300, bbox_inches='tight')
    ...
    plt.title('SOC Change from Initial Value', fontsize=16)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    plt.savefig("comparison_results/soc_delta.png", dpi=300, bbox_inches='tight')
...
def main():
    ...
    print(f"Results saved in: {os.path.abspath('comparison_results')}")
